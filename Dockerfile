...
ENV domogik_release=0.5-hotfix
...
RUN apt-get install -y \
    ...
    python2.7-dev \
    python-pip \
    git libzmq5 libffi-dev libssl-dev
...
RUN pip install alembic SQLAlchemy-Utils tornado pytz pymysql chardet psutil requests
...
RUN /bin/bash -c "/usr/bin/mysqld_safe &" \
 && sleep 5 \
 && cd /opt/dmg/domogik \
 && python install.py --user domogik --command-line  --admin_interfaces "*"  --database_host "192.168.1.81"
...
RUN cd /opt/dmg && \
    ...
    cd domogik-plugin-velbus && \
    git checkout develop && \
    su - domogik -c "dmg_package -i /opt/dmg/domogik-plugin-velbus" 
